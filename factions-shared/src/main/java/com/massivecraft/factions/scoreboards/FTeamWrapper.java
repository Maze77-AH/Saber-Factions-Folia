...
public class FTeamWrapper {
    ...
    public static void applyUpdates(Faction faction) {
        ...
        if (!FactionsPlugin.getInstance().getConfig().getBoolean("scoreboard.default-prefixes", false)) return;
    ...
    private void updatePrefix(FScoreboard fboard) {
        if (FactionsPlugin.getInstance().getConfig().getBoolean("scoreboard.default-prefixes", false)) {
            ...
            team.setCanSeeFriendlyInvisibles(FactionsPlugin.getInstance().getConfig().getBoolean("See-Invisible-Faction-Members", false));
