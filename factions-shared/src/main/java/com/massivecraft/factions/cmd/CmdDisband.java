...
public class CmdDisband extends FCommand {
    ...
    public void perform(CommandContext context) {
        ...
        boolean canDisbandAny = Permission.DISBAND_ANY.has(context.sender, false);
        boolean canManageAdminFaction = faction.isAdminFaction() && Permission.CREATE_ADMIN.has(context.sender, false);

        if (!isMyFaction && !Permission.DISBAND_ANY.has(context.sender, true) && !canManageAdminFaction) return;

        if (context.fPlayer != null && !context.fPlayer.isAdminBypassing() && !canDisbandAny && !canManageAdminFaction && !hasDisbandPermission(context, faction)) {
        ...
        if (faction.isPermanent() && !canDisbandPermanentFaction(context, canDisbandAny, canManageAdminFaction)) {
    ...
    private boolean canDisbandPermanentFaction(CommandContext context, boolean canDisbandAny, boolean canManageAdminFaction) {
        return context.player == null || canDisbandAny || canManageAdminFaction || (context.fPlayer != null && context.fPlayer.isAdminBypassing());
    }
    ...
    private void promptDisbandConfirmation(CommandContext context, Faction faction) {
        if (!disbandMap.containsKey(context.player.getUniqueId().toString()) && faction.getTnt() > 0) {
            ...
            // Pure confirmation-map cleanup (no Bukkit access); run on the global scheduler.
            FactionsPlugin.getInstance().getFactionScheduler().runGlobalLater(() -> disbandMap.remove(context.player.getUniqueId().toString()), 200L);
