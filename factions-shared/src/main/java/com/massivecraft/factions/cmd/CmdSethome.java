...
public class CmdSethome extends FCommand {
    ...
    public void perform(CommandContext context) {
        ...
        // Capture the location on the (region-owning) command thread, then perform the faction
        // home write on the single-writer model thread.
        final org.bukkit.Location home = context.player.getLocation();
        final Faction homeFaction = faction;
        FactionsPlugin.getInstance().getRealFactionsServices().executor().runFactionWrite(() -> {
            homeFaction.setHome(home);

            homeFaction.msg(TL.COMMAND_SETHOME_SET, context.fPlayer.describeTo(context.faction, true));
            homeFaction.sendMessage(FactionsPlugin.getInstance().cmdBase.cmdHome.getUsageTemplate(context));
            if (homeFaction != context.faction) {
                context.msg(TL.COMMAND_SETHOME_SETOTHER, homeFaction.getTag(context.fPlayer));
            }
        });
    }
