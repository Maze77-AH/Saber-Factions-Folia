...
public abstract class MemoryFPlayer implements FPlayer {
    ...
    protected transient UUID cachedUniqueId;
    protected transient boolean uniqueIdResolved;
    protected transient Player cachedPlayer;
    ...
    public int getKills() {
        Player player = getPlayer();
        return player != null ? player.getStatistic(Statistic.PLAYER_KILLS) : this.kills;
    }

    public int getDeaths() {
        Player player = getPlayer();
        return player != null ? player.getStatistic(Statistic.DEATHS) : this.deaths;
    }
    ...
    public void onDeath() {
        ...
        Player player = this.getPlayer();
        if (player != null && player.hasMetadata("diedToPlayer")) {
            this.alterPower(-Conf.deathToPlayerPowerLoss);
            player.removeMetadata("diedToPlayer", FactionsPlugin.getInstance());
        }
    ...
    public boolean canClaimForFaction(Faction forFaction) {
        Player player = this.getPlayer();
        boolean canManageSafeZone = player != null && Permission.MANAGE_SAFE_ZONE.has(player);
        boolean canManageWarZone = player != null && Permission.MANAGE_WAR_ZONE.has(player);
        return this.isAdminBypassing() || !forFaction.isWilderness() && (forFaction == this.getFaction() && this.getRole().isAtLeast(Role.MODERATOR)) || (forFaction.isSafeZone() && canManageSafeZone) || (forFaction.isWarZone() && canManageWarZone);
    }
    ...
    public boolean canClaimForFactionAtLocation(Faction forFaction, FLocation flocation, boolean notifyFailure) {
        ...
        } else if (isAdminBypassing() || forFaction.isSafeZone() && canManageSafeZone || forFaction.isWarZone() && canManageWarZone) {
            return true;
    ...
    public boolean attemptUnclaim(Faction forFaction, FLocation flocation, boolean notifyFailure) {
        ...
        if (targetFaction.isSafeZone() && (player == null || !Permission.MANAGE_SAFE_ZONE.has(player))) {
            msg(TL.COMMAND_UNCLAIM_SAFEZONE_NOPERM);
            return false;
        }

        if (targetFaction.isWarZone() && (player == null || !Permission.MANAGE_WAR_ZONE.has(player))) {
    ...
    public Player getPlayer() {
        ...
        Player player = this.cachedPlayer;
        if (player != null) {
            if (player.isOnline() && uniqueId.equals(player.getUniqueId())) {
                return player;
            }
            this.cachedPlayer = null;
        }

        this.cachedPlayer = Bukkit.getPlayer(uniqueId);
    ...
    public boolean canFlyAtLocation(FLocation location) {
        ...
        } else if (faction.getRelationTo(getFaction()) == Relation.ENEMY && Permission.FLY_ENEMY.has(player)) {
            return true;
        } else if (faction.getRelationTo(getFaction()) == Relation.ALLY && Permission.FLY_ALLY.has(player)) {
            return true;
        } else if (faction.getRelationTo(getFaction()) == Relation.TRUCE && Permission.FLY_TRUCE.has(player)) {
            return true;
        } else if (faction.getRelationTo(getFaction()) == Relation.NEUTRAL && !faction.isSystemFaction() && Permission.FLY_NEUTRAL.has(player)) {
            return true;
        }
    ...
    protected void bindPlayer(Player player) {
        ...
        this.cachedUniqueId = player.getUniqueId();
        this.uniqueIdResolved = true;
    }

    protected void unbindPlayer(Player player) {
        if (player == null || this.cachedPlayer == player) {
            this.cachedPlayer = null;
        }
    }

    private UUID getUniqueId() {
        if (!this.uniqueIdResolved) {
            this.uniqueIdResolved = true;
            ...
            try {
                this.cachedUniqueId = FastUUID.parseUUID(currentId);
    ...
    public Boolean canflyinWilderness() {
        Player player = getPlayer();
        return player != null && player.hasPermission(Permission.FLY_WILDERNESS.node);
    }

    @Override
    public Boolean canflyinWarzone() {
        Player player = getPlayer();
        return player != null && player.hasPermission(Permission.FLY_WARZONE.node);
    ...
    public Boolean canflyinSafezone() {
        Player player = getPlayer();
        return player != null && player.hasPermission(Permission.FLY_SAFEZONE.node);
    ...
    public Boolean canflyinEnemy() {
        Player player = getPlayer();
        return player != null && player.hasPermission(Permission.FLY_ENEMY.node);
    ...
    public Boolean canflyinAlly() {
        Player player = getPlayer();
        return player != null && player.hasPermission(Permission.FLY_ALLY.node);
    ...
    public Boolean canflyinTruce() {
        Player player = getPlayer();
        return player != null && player.hasPermission(Permission.FLY_TRUCE.node);
    ...
    public Boolean canflyinNeutral() {
        Player player = getPlayer();
        return player != null && player.hasPermission(Permission.FLY_NEUTRAL.node);
    ...
    public boolean takeMoney(long amt) {
        if (hasMoney(amt)) {
            Economy econ = FactionsPlugin.getInstance().getEcon();
            Player player = getPlayer();
            if (player != null && econ.withdrawPlayer(player, amt).transactionSuccess()) {
